...
@section('content')
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="lg:col-span-2 bg-white border rounded-xl shadow-sm p-6">
            ...
            @php
            $stepMeta = [
                'month'      => ['icon' => '🗓', 'label' => 'انتخاب ماه گزارش',   'desc' => 'کاربر از بین ۳ ماه اخیر انتخاب می‌کند',    'color' => 'bg-blue-50 border-blue-200'],
                'department' => ['icon' => '🏢', 'label' => 'انتخاب دپارتمان',    'desc' => 'کاربر دپارتمان مربوطه را انتخاب می‌کند',   'color' => 'bg-orange-50 border-orange-200'],
                'category'   => ['icon' => '📂', 'label' => 'انتخاب دسته‌بندی',   'desc' => 'نوع گزارش (فرم‌ساز) مشخص می‌شود',         'color' => 'bg-purple-50 border-purple-200'],
            ];
            @endphp
            ...
            <div id="flow-outer" dir="ltr" class="flex flex-col sm:flex-row sm:gap-10 items-stretch">
                ...
                <div id="flow-steps-sortable"
                    ...
                    @foreach($flowSteps as $i => $stepKey)
                    @php $meta = $stepMeta[$stepKey]; @endphp
                    <div dir="rtl" class="flow-step-item flex-1 relative border-2 rounded-xl p-4 {{ $meta['color'] }} cursor-default select-none"
                         data-step="{{ $stepKey }}">

                        <span class="flow-step-number absolute -top-3 -right-2 size-6 flex items-center justify-center bg-white border-2 border-gray-300 rounded-full text-xs font-bold text-gray-600">{{ $i + 1 }}</span>
                        ...
                        <div class="flex items-start gap-3" dir="rtl">
                            ...
                            <div>
                                ...
                                <div class="font-semibold text-gray-800 text-sm">{{ $meta['label'] }}</div>
