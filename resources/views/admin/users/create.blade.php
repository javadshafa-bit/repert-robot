...
@section('content')
<div class="mb-6">
    <h2 class="text-2xl font-bold text-gray-800">افزودن کاربر جدید</h2>
    <a href="{{ route('admin.users.index') }}" class="text-sm text-blue-600 hover:underline">← بازگشت</a>
</div>

<form action="{{ route('admin.users.store') }}" method="POST" class="max-w-xl space-y-5">
    @csrf
    <div class="bg-white border rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div>
            <label class="block text-sm font-medium mb-1">رمز عبور <span class="text-red-500">*</span></label>
    ...
    <div class="bg-white border rounded-xl shadow-sm p-6">
        <h3 class="font-semibold text-gray-700 border-b pb-2 mb-4">تخصیص نقش</h3>
        @forelse($roles as $role)
        <label class="flex items-start gap-3 p-3 border rounded-lg mb-2 cursor-pointer hover:bg-indigo-50 has-[:checked]:border-indigo-400 has-[:checked]:bg-indigo-50">
        ...
        @empty
            <p class="text-sm text-gray-400">هیچ نقشی تعریف نشده. ابتدا <a href="{{ route('admin.roles.create') }}" class="text-blue-600 underline">نقش بسازید</a>.</p>
        @endforelse
    </div>
