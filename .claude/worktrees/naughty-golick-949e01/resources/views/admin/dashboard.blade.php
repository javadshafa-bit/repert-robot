...
@section('content')
    ...
    <div class="grid lg:grid-cols-3 gap-4 mb-4">
        ...
        <div class="lg:col-span-2 bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">آخرین ماه‌های ثبت‌شده</p>
                </div>
                <div class="flex-shrink-0 size-8 bg-blue-50 rounded-lg flex items-center justify-center">
        ...
        <!-- نمودار دونات: وضعیت اتصال نمایندگان -->
        <div class="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div>
                    <h3 class="text-sm font-bold text-gray-800">وضعیت اتصال نمایندگان</h3>
                    <p class="text-xs text-gray-400 mt-0.5">متصل در برابر غیرمتصل</p>
                </div>
                <div class="flex-shrink-0 size-8 bg-emerald-50 rounded-lg flex items-center justify-center">
                    <svg class="size-4 text-emerald-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">سهم هر دپارتمان از کل گزارش‌ها</p>
                </div>
        ...
        <div class="bg-white border border-gray-200 shadow-sm rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">پرتکرارترین دسته‌بندی‌ها</p>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const chartColors = ['#6366f1', '#10b981', '#f59e0b', '#ef4444', '#3b82f6', '#8b5cf6', '#06b6d4', '#f97316'];

    // ===== نمودار ستونی: گزارش‌ها بر اساس ماه =====
    @if($reportsByMonth->isNotEmpty())
    (function () {
        const months = @json($reportsByMonth->pluck('jalali_month'));
        const counts = @json($reportsByMonth->pluck('total'));
        ...
        new ApexCharts(document.getElementById('chart-reports-month'), {
            ...
            xaxis: {
                categories: months,
                labels: { style: { colors: textColor, fontFamily: fontFamily }, rotate: -25 }
            },
            yaxis: {
            ...
            dataLabels: {
                enabled: true,
                offsetY: -18,
                style: { fontSize: '11px', fontFamily: fontFamily, colors: [textColor] }
            },
    ...
    @if($totalReps > 0)
    (function () {
        const connected    = {{ $connectedReps }};
        const disconnected = {{ $totalReps - $connectedReps }};

        new ApexCharts(document.getElementById('chart-reps-status'), {
            chart: {
                type: 'donut',
                height: 240,
                fontFamily: fontFamily,
            ...
            plotOptions: {
                pie: { donut: { size: '68%', labels: {
                    ...
                    value: { fontFamily: fontFamily, color: textColor }
                }}}
            },
            dataLabels: { enabled: false },
            legend: { position: 'bottom', fontFamily: fontFamily, labels: { colors: textColor } },
            tooltip: { style: { fontFamily: fontFamily } }
        }).render();
    })();
    @endif
    ...
    (function () {
        const deptLabels = @json($reportsByDept->pluck('name'));
        const deptCounts = @json($reportsByDept->pluck('total'));
    ...
    (function () {
        const catLabels = @json($reportsByCategory->pluck('name'));
        const catCounts = @json($reportsByCategory->pluck('total'));
        ...
        new ApexCharts(document.getElementById('chart-reports-category'), {
            ...
            series: [{ name: 'تعداد گزارش', data: catCounts }],
            xaxis: {
